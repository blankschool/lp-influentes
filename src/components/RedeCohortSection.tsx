    ...
    <div className="flex flex-col mt-12" style={{ border: '1px solid rgba(240,237,230,0.08)' }}>
      {rows.map(([k, v], i) => (
        <div key={i} className="grid grid-cols-2" style={{ borderBottom: i < rows.length - 1 ? '1px solid rgba(240,237,230,0.08)' : 'none' }}>
          <div className="py-[18px] px-6 text-[13px] font-light" style={{ color: 'rgba(240,237,230,0.28)', borderRight: '1px solid rgba(240,237,230,0.08)' }}>{k}</div>
          <div className="py-[18px] px-6 text-[13px] font-medium" style={{ color: 'rgba(240,237,230,0.65)' }}>{v}</div>
        </div>
      ))}
    </div>
  </section>
